...
- (void)reset
{
    ...
    [self.table selectItemWithIndex:0];
    
    [self.delegate didSelectItemAtIndex:0];
    
    self.menuButton.title.text = self.items[0];
...
- (void)onShowMenu
{
    ...
    {
        [self.table selectItemWithIndex:self.currentIndex];
    }
...
- (void)didSelectItemAtIndex:(NSUInteger)index withTitle: (NSString *) title
{
